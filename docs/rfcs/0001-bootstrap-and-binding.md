...
| Client `pre-push` hook (`core.hooksPath`) | Block `git push origin main`, block topic pushes to `projection`, require FF | `--no-verify`; not installed until clone/bootstrap; not on every GUI |
